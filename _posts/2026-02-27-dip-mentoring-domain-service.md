...
class OrderDomainService {
    ...
    fun createOrderItems(
        ...
        itemCommands: List<CreateOrderItemCommand>
    ): List<OrderItem> {
        return itemCommands.map { command ->
            val product = products.find { it.id == command.productId }
                ?: throw CoreException(ErrorType.NOT_FOUND)
            product.validateOrderable()
            OrderItem.create(product, command.quantity)
        }
    }

    fun calculateTotalAmount(items: List<OrderItem>): Long {
...
```kotlin
@Test
fun `재고가 부족하면 주문 아이템을 생성할 수 없다`() {
    val product = Product.create(name = "테스트", price = 1000, stock = 1)
    val commands = listOf(CreateOrderItemCommand(productId = product.id, quantity = 5))

    assertThrows<CoreException> {
        orderDomainService.createOrderItems(listOf(product), commands)
    }
}
```

D 멘토가 틀린 게 아니고, L 멘토가 틀린 것도 아니다. 선이 다를 뿐이다. 그리고 그 선에 따라 테스트 전략이 달라진다.
...
> 의존 방향만 지키면, 디테일은 선택이다. 단, 왜 그렇게 선택했는지 설명할 수 있어야 한다.
